 ...
 *  Distance to goal heuristic = Euclidean.
 *  This code assumes truncating conversion from double to int
 *
 *  compile as follows: gcc fielddstar_gradient_extract.c -lglut
 *
 */
...
// for visualization, also returns a flag denoting path type
double* computeCostBP(node* s, node* s_a, node* s_b)
{
  ...
  double y = 1;
  double x = 1;
  double path_type = 1; // used as a flag flag, 1 = direct, 2 = more than one path segment
  int b_y, b_x;
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if(x != 1) // then path spends some time in both grids b and c 
          path_type = 2;
      } 
    } 
  }
...
// this is procedure computeShortestPath from the paper (final version), it does the meat of the planning/replanning
...
//printf("here 1 \n");
    ...
    // the following case is now implicity handeled in topKey()
    //if(s == NULL) // the heap is empty, I added this to fix a rare case that happens when start_s is the only node in the heap, and then it is popped and no other nodes are inserted
    //{
    //  printf(" the heap is empty, returning\n");
    //  getchar();
    //  return;
    //}
    ...
    else
    {
      ...
      for(n = 0; n < NEIGHBORS; n++)
      {
        ...
        #ifdef R0GOAL
        if((s_prime->bestNeighbor == s || s_prime->bestNeighbor == cknbr(s_prime,s)) && ((double)s_prime->x != goalRM || (double)s_prime->y != goalRN)) // I added the check for the goal         
        #else
...
//{
//  printf("prime null \n");
//  printf("here 18 goal: %d start: %d prime: %d \n", s_goal, s_start, s_prime);
//}
//else
//{
//  if(s_prime->bestNeighbor == NULL)
//    printf("it is null \n");
//  else
//    printf("here 18 goal: %d start: %d prime: %d s->bn: %d \n", s_goal, s_start, s_prime, s_prime->bestNeighbor);
//}
...
void updadeCell(int y, int x, double newCost)
{
  ...
  #ifdef R2GOAL
    if(((int)goalRM == x) || (((double)((int)goalRM)) == goalRM && (int)goalRM-1 == x )) // don't update goal neighbors
      if(((int)goalRN == y) || (((double)((int)goalRN)) == goalRN && (int)goalRN-1 == y )) // don't update goal neighbors
        return;        
  #endif 
...
void computePointCostToEdge(double y_hat, double x_hat, node* s, node* s_a, node* s_b, CellPath** subPaths)
{
  ...
  else if(s_1->g > s_2->g+c) // to avoid getting imaginary answer, need to give this an appropriate g based on s_2
    s_1->g = s_2->g + c;   
  else if(s_2->g > s_1->g+c) // to avoid getting imaginary answer, need to give this an appropriate g based on s_1
    s_2->g = s_1->g + c; 
...
{  
  ...
  {   
    ...
    drawPathOnMaze(robotsPath, clrG, .75); // draw the path
    ...
    while(1 == 1)
    {
      ...
      /*if(lookAhead_pos_y == (double)((int)lookAhead_pos_y) && lookAhead_pos_x == (double)((int)lookAhead_pos_x)) // look ahead point is on a corner
      {
        ...
        { 
          ...
          {
              printf(" what do you know, we are here 1\n");  // reinsert into heap with new point and cost based on look ahead
              getchar();
...
{  
  ...
  { 
    ...
    else if(at_x == (double)int_at_x && at_y != (double)int_at_y)
    {
      ...
      if(low_x == high_x && high_y - low_y < 1 && low_path_type != 2 && high_path_type !=2)
      {
        ...
        else if(pos_x == (double)int_pos_x && pos_y == (double)int_pos_y && map[(int)min(pos_y,at_y)][(int)min(pos_x,at_x)] < map[(int)min(pos_y,at_y)][(int)temp_x])
        {
          ...
          // so if that happense, we don't use the gradient stuff.
        }
        else
        {
    ...
    else // follow back pointer directly
    {
      ...
      if(graph[int_at_y][int_at_x].bestNeighbor != NULL)
      {
        ...
        if(anotherBest != NULL)
        {
          ...
          pos_y = (double)int_at_y+endPoints[0];
          pos_x = (double)int_at_x+endPoints[1];
        }
      }   
    ...
    // because neither 1-step look ahead nor gradient method were used to get the last point (note this throws off the visuilization path cost)
    int p;
...
// returns the row with the min val at column Col in map[][]
int row_with_min_val(int Col)
{
  int min_row = 0;
  double min_val = map[0][Col];
  int i;

  for(i = 1; i < HEIGHT; i++)
  {
    if(map[i][Col] < map[min_row][Col])
      min_row = i;
  }
  return min_row;
}
...
int main(int argc, char *argv[])
{
  ...
  time_t rand_seed;
  (void) time(&rand_seed);
  this_seed = (long) rand_seed;
  //this_seed = 1255971402;  
  srandom(this_seed);
  fflush(stdout);  

  printf("seed: %d \n", (long) this_seed); // in case there is an error, remember random seed
 ...
 //               1, 1, 1, 1, 1, 1, 1, 1, 6, 5};
   ...
   #endif
   //int robotN = row_with_min_val(5*WIDTH/6); //HEIGHT/2;
   int robotN = HEIGHT/2;
   int robotM = 5*WIDTH/6;
   s_start = &graph[robotN][robotM];
   

   //thresholdMap(0,55000,1);
   //thresholdMap(1,100000,100000);


   //thresholdMap(0,40000,1);
   //thresholdMap(1,60000,5);
   //thresholdMap(5,80000,10);
   //thresholdMap(10,100000,20);
 ...
 //  double path4_cost = calculatePathCost(pathToGoalMine2);
   ...
   if (DISPLAYGRAPHICS == 1)
   {
     ...
     //drawPathOnMaze(pathToGoal, clrG, .75); // draw the path
     //drawPathOnMaze(pathToGoalWithLookAhead, clrB, .76); // draw the path
 ...
 //    deleteMapPath(pathToGoalMine2);
     ...
     {
       updadeCell(anUpdateList->rows[i], anUpdateList->columns[i], anUpdateList->obstacle[i]); // may need to modify both ends of the vertex
       //checkHeap();
       //pauseFor(.2);
       //getchar();  
     }
...
/*
   ...
   FILE *fpa;
   fpa = fopen("1000_2.txt","a");
   fprintf(fpa,"%f %f %f %f %f %f %f %f %f\n", path1_cost, path2_cost, path3_cost, path4_cost, path5_cost, path2_reduction, path3_reduction, path4_reduction, path5_reduction);
   fclose(fpa);
